...
tillbaka nollställer filtreringen, eftersom en bar navigering till
startsidan (utan några queryparametrar alls) alltid tolkats som "inga
filter". Med antalet
...
efter tillägg/redigering/borttagning (som alltid går till startsidan
utan några queryparametrar), en klickad "Avbryt"-länk, eller en helt
vanlig sidladdning.

"Antal flaskor minst"-filtret löste ett näraliggande men INTE
...
taggar - uttryckligen INTE "antal flaskor minst"-filtret, som behåller
sitt egna, oförändrade databasmönster) sparas i HTTP-sessionen, inte i
databasen. Sessionen är rätt livslängd för det här beslutet: texten
"tills användaren aktivt väljer att ändra den" beskriver ett minne för
det pågående besöket, inte en varaktig, enhetsöverskridande inställning
som "antal flaskor minst"-filtret är. En session är också redan en
etablerad plats för tillfälligt, per-besök-tillstånd i den här appen
(se t.ex. den temporära importsökvägen, som också bara lever i
sessionen).

En request utan någon egen filtreringsparameter alls (en bar navigering
till startsidan) använder sessionens senast sparade filtrering om en
sådan finns, annars kodens vanliga standardvärden - exakt samma första
intryck som idag för en helt ny session. En enskild explicit
queryparameter (t.ex. en delad länk med bara ett sökord) åsidosätter
bara sitt eget fält och blir själv det nya ihågkomna värdet för just
det fältet - samma princip "antal flaskor minst"-filtret redan följer,
fast nu applicerad på hela filtreringen.
...
tidigare på startsidan helt utan queryparametrar - vilket med det här
beslutet bara hade returnerat exakt samma ihågkomna filtrering de
skulle ta bort. De fick
...
- En användare kan navigera fritt mellan vinlistan och andra sidor
  ...
  redirectar till startsidan utan några queryparametrar).
- Filtreringen återställs till vanliga standardvärden vid en ny session
  ...
  enhetsöverskridande, till skillnad från "antal flaskor minst"-filtret.
- De två "rensa"-länkarna bär numera en egen signal för att skilja
  "töm minnet" från "läs minnet" - en bar navigering till startsidan
  och samma navigering med den signalen beter sig olika trots att
  ingen av dem bär någon egen filtreringsparameter.
